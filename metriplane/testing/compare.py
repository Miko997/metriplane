...
def compare_incidents(
    observed: list[IncidentRecord],
    specs: list[ExpectedIncidentSpec],
    strict_extra: bool = False,
    rule_types: dict[str, str] | None = None,
) -> list[dict[str, Any]]:
    ...
    for spec in specs:
        hits = [
            i for i, inc in enumerate(observed) if _incident_matches(inc, spec, known_rule_types)
        ]
        ...
        checks.append(
            _check(
                f"incident[{spec.type}/{spec.rule_id or '*'}]", ok, f"matched {n} (expected {rng})"
            )
        )
    if strict_extra:
...
def compare_events(
    observed: list[RuleAlert], specs: list[ExpectedEventSpec], strict_extra: bool = False
) -> list[dict[str, Any]]:
    checks: list[dict[str, Any]] = []
    matched_idx: set[int] = set()
    for spec in specs:
        ...
        checks.append(
            _check(
                f"event[{spec.rule_id or spec.event_type or '*'}]",
                ok,
    ...
    if strict_extra:
        extra = sorted(
            observed[index].rule_id for index in range(len(observed)) if index not in matched_idx
        )
        checks.append(
            _check(
                "events.no_extra",
...
def compare_latency(
    p95_ms: float | None, expected: PhysicalRegressionExpected
) -> list[dict[str, Any]]:
    ...
    ok = p95_ms <= limit
    return [_check("latency.p95_update_ms", ok, f"{round(p95_ms, 3)}ms (max {limit}ms)")]
