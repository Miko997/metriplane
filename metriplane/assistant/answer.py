...
def _incident_search(q: AssistantQuery, run: Path) -> AssistantAnswer:
    ...
    if not incidents:
        return AssistantAnswer(
            ...
            citations=cites,
            confidence=0.6 if not lims else 0.2,
            limitations=lims,
        )
...
def _object_history(q: AssistantQuery, run: Path) -> AssistantAnswer:
    ...
    r = rows[0]
    ans = (
        f"{target} was tracked for {r['duration_s']}s, traveling "
...
def _rule_explanation(q: AssistantQuery, run: Path) -> AssistantAnswer:
    ...
    target = next((rid for rid in candidates if rid and rid.lower() in q.question.lower()), None)
    if target is None:
    ...
    ans = (
        f"Rule {rule.get('id')} is a {rule.get('type')} rule "
...
def _camera_health(q: AssistantQuery, run: Path) -> AssistantAnswer:
    ...
    if bad:
        details = "; ".join(
            ...
            f"dropout {scores[cid].get('dropout_rate')})"
            for cid in bad
        )
