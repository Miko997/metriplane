...
def retrieve_incidents(
    ...
    cites = [make_citation(p, run, "incidents", record_id=i.get("incident_id")) for i in incidents]
    return incidents, cites, []


def retrieve_rule(
    run_dir: str | Path, rule_id: str | None
    ...
    if rule_id is None:
        return ({"rules": [r.get("id") for r in rules]}, [make_citation(p, run, "rules")], [])
    for r in rules:
        if r.get("id") == rule_id:
...
def retrieve_traces(
    run_dir: str | Path, object_id: str | None = None
) -> tuple[list[dict[str, Any]], list[CitationModel], list[str]]:
    run = Path(run_dir)
    session = _find(run, ["session_excerpt.jsonl", "session.jsonl"])
    if session is None:
