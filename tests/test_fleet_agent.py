...
def test_heartbeat_serializes():
    hb = FleetHeartbeat(
        node_id="cam_node_01", ts=1.0, run_id="r", git_commit="abc",
        config_hash="def", health_overall="OK", fps=30.0, objects_tracked=3,
        active_incidents=1, frames_total=100, frames_dropped_total=0)
...
def test_emit_once_writes_jsonl(tmp_path):
    ...
    agent = FleetAgent(
        FleetAgentConfig(node_id="cam_node_01", output_path=str(out),
                         run_id="fleet_test", git_commit="abc123"),
        metrics_provider=lambda: {"health_overall": "OK", "fps": 30.0,
                                  "objects_tracked": 3, "active_incidents": 1})
    hb = agent.emit_once()
...
def test_heartbeat_includes_provenance():
    agent = FleetAgent(FleetAgentConfig(node_id="n", run_id="r1",
                                        git_commit="c1", config_hash="h1"))
