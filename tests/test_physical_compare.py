...
def inc(rule_id="no_cart_in_exit_lane", severity="warning",
        objects=("cart_01",), zones=("exit_lane",)):
    return IncidentRecord(
        incident_id="inc_0001", rule_id=rule_id, severity=severity,
        status="closed", opened_ts=1.0, closed_ts=2.0, duration_s=1.0,
        object_ids=list(objects), zones=list(zones), summary="x")
...
def test_event_count_matching():
    alerts = [RuleAlert(alert_id=f"a{i}", rule_id="r", severity="warning",
                        ts=float(i), object_ids=["cart_01"]) for i in range(3)]
    checks = compare_events(alerts, [ExpectedEventSpec(rule_id="r", min_count=2)])
    assert passed(checks)


def test_event_count_too_few_fails():
    alerts = [RuleAlert(alert_id="a", rule_id="r", severity="warning",
                        ts=0.0, object_ids=[])]
