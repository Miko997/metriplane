...
def _pkg(rules, subjects=None):
    return SpatialContractPackage(
        ...
        subjects=subjects
        or {
            "movable": SubjectSpec(object_types=["cart"]),
...
def test_valid_forbidden_zone():
    pkg = _pkg(
        [ContractRuleSpec(id="r1", type="forbidden_zone", subject="movable", zones=["exit_lane"])]
    )
...
def test_minimum_distance_requires_distance():
    with pytest.raises(ValueError):
        _pkg(
            [
                ContractRuleSpec(
                    id="r", type="minimum_distance", subject_a="people", subject_b="movable"
                )
            ]
        )
...
def test_zone_duration_requires_max_duration():
    with pytest.raises(ValueError):
        _pkg(
            [
                ContractRuleSpec(
                    id="r", type="zone_occupancy_duration", subject="movable", zones=["exit_lane"]
                )
            ]
        )
...
def test_duplicate_rule_ids_fail():
    with pytest.raises(ValueError, match="duplicate rule id"):
        _pkg(
            [
                ContractRuleSpec(id="dup", type="forbidden_zone", subject="movable", zones=["z"]),
                ContractRuleSpec(id="dup", type="forbidden_zone", subject="movable", zones=["z"]),
            ]
        )
...
def test_unknown_subject_reference_fails():
    with pytest.raises(ValueError, match="unknown subject"):
        _pkg([ContractRuleSpec(id="r", type="forbidden_zone", subject="ghost", zones=["z"])])
...
def test_negative_cooldown_rejected():
    with pytest.raises(ValueError):
        _pkg(
            [
                ContractRuleSpec(
                    id="r", type="forbidden_zone", subject="movable", zones=["z"], cooldown_s=-1.0
                )
            ]
        )
...
def test_forbidden_direction_requires_allowed_direction():
    with pytest.raises(ValueError):
        _pkg(
            [
                ContractRuleSpec(
                    id="r", type="forbidden_direction", subject="movable", zones=["packing"]
                )
            ]
        )
