...
from metriplane.sentinel.engine import evaluate_session, iter_frames
    ...
    rules = RuleSet(
        rules=[
            RuleDefinition(
                id="pallet_dwell",
                type="max_dwell",
                object_filter=ObjectFilter(type="pallet"),
                zone="exit_lane",
                max_duration_s=5.0,
                severity="critical",
            )
...
def test_fixed_clock_is_authoritative_for_rule_duration(tmp_path, registry):
    ...
    records = [
        json.loads(make_frame(100.0, 0, [{"id": 12, "pos": [1.0, 0.0, 0.0], "zone": "exit_lane"}])),
        json.loads(make_frame(101.0, 1, [{"id": 12, "pos": [1.0, 0.0, 0.0], "zone": "exit_lane"}])),
    ]
...
def test_run_id_captured(tmp_path, registry):
    ...
    session = write_session(
        ...
        [
            make_frame(
                0.0, 0, [{"id": 7, "pos": [0.0, 0.0, 0.0], "zone": "exit_lane"}], run_id="my_run"
            ),
        ],
    )
...
def test_fallback_object_id_without_registry(tmp_path):
    rules = RuleSet(rules=[RuleDefinition(id="r", type="forbidden_zone", zone="exit_lane")])
    session = write_session(
        tmp_path,
        [
