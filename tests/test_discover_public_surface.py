...
def test_production_census_is_exact_and_family_closed(
    ...
    assert len(production_discovery.rows) == 10_939
    assert sum(production_discovery.family_counts.values()) == 10_939
    assert production_discovery.family_counts["public_api"] == 2_514
    assert production_discovery.family_counts["manifest_keys"] == 3_938
    assert production_discovery.family_counts["resources"] == 1_632
...
def test_manifest_projection_is_exactly_json_csv_models_and_hook(
    ...
    assert counts == {"json": 2807, "csv": 9, "model": 35, "hook": 1087}
...
def test_direct_script_loads_sibling_kernel_without_pythonpath() -> None:
    ...
    assert summary["rows"] == 10_915
    assert summary["public_api"] == 2_514
...
def test_report_binds_materialization_and_denies_runtime_claim(
    ...
    assert "Owned rows: `10915`" in text
