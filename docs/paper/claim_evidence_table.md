...
|---|---|---|---|
| v0.2.0 is the current SoftwareX paper artifact and release candidate | Supported | `README.md`, `ARTIFACTS.md`, `docs/paper/reproduction.md` | It is not tagged or archived on Zenodo yet. |
...
| Docker dummy-mode local smoke was captured | Supported as bounded smoke only | `evidence/paper_v2_0/logs/16_docker_demo_up.txt`, `evidence/paper_v2_0/logs/17_docker_health.json`, `evidence/paper_v2_0/logs/18_docker_clean.txt` | Build/start, health endpoint JSON, and cleanup logs only. Not benchmark, production-runtime, live-camera, replay-mode, reliability, or safety evidence. |
| Integration adapters exist | Supported with limits | `docs/release_v0_2_claims.md`, integration docs and evidence files | Isaac Sim remains NOT RUN; Docker is bounded to dummy-mode local smoke; ROS 2/Omniverse claims remain bounded to existing manual/export evidence. |
