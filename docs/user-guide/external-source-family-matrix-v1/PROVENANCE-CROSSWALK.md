...
| ROS 2 / MCAP + TF2 | **NOT TESTED** | Planned only: an immutable recording plus message definitions and TF/calibration sidecars would be required | No topic synchronization, TF materialization, calibration, or snapshot policy exists | None | None; no ROS 2 or MCAP compatibility claim exists. A future profile is not part of this publication |
| MassRobotics AMR offline replay | **PARTIALLY SUPPORTED** | Metriplane-authored synthetic identity/status JSONL for two AMRs; upstream standard identities are reference-only and upstream bytes are absent | None | Validate source UUIDs, strict UTC timestamps, current-location XYZ/quaternion/datum, and optional path datums; derive elapsed time, stable order, identity-bound target positions, and inclusive zone assignment; never promote predictions | Nine complete two-object frames per variant, then a separate operator-authored rendezvous rule. Incident `9/4/1/1`; control `9/3/0/0`. The result is an owner-generated format-engineering artifact, not external-source or conformance evidence |
