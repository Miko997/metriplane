...
# v0.4.0 migration and behavior changes

v0.4.0 is the reduced Truth Recovery core release. It adds governed discovery,
bounded external recorded-state evaluation, portable output-path hardening, and
release-integrity controls. It does not complete the later Assurance Hardening
programme.
...
An external manifest binds the exact Metriplane evaluation version. v0.4.0 fails
closed when `evaluation.metriplane_version` names a different installed version.
A v0.3.0 fixture or proof record is therefore not silently promoted to v0.4.0.

Keep an existing source-specific v0.3.0 fixture tree and its fingerprints
unchanged. To evaluate the same normalized material under v0.4.0, create a
...
fresh v0.4.0 candidate cells recorded in the finalized release results.

Historical v0.3.0 WSL2 and native-Windows observations remain historical. They
must not be relabeled as v0.4.0 validation or broadened into platform support.
...
v0.4.0. Historical v2.5.x authority packets are not prerequisites or evidence
...
v0.4.0 does not produce or replace the frozen SoftwareX evidence or historical
...
The manuscript at `10.2139/ssrn.7166858` is cited separately. No v0.4.0 DOI is
claimed.
