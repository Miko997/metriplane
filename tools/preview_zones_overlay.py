...
def main() -> int:
    ...
    zones_path = args.zones or (calib.zones if calib is not None else None)

    if mapping_path is None:
        ap.error("Need --mapping OR (--profile / calib/active_profile.yaml).")
    if zones_path is None:
        ap.error("Need --zones OR (--profile / calib/active_profile.yaml).")
    ...
    if calib is not None:
        print(f"[preview_zones] profile={calib.profile} dir={calib.profile_dir}")
    print(f"[preview_zones] mapping={mapping_path}")
    print(f"[preview_zones] zones={zones_path}")
    print(f"[preview_zones] intrinsics={intr_path if intr_path else '(none)'}")
    ...
    try:
        while True:
            ...
            cv2.imshow("metriplane_m6_zones_preview", vis)
            if writer is not None:
                writer.write(vis)

            if (time.time() - last_print) >= 1.0 and lines:
                print("[preview_zones]", " | ".join(lines[:6]))
                last_print = time.time()
