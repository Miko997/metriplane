...
def _resolve_multi_mapper_from_cfg(cfg: Config) -> tuple[dict[str, int], MultiPlanarMapper]:
    ...
    mm = load_multi_planar_mapper(
        mapping_by_camera=mapping_by_cam, intrinsics_by_camera=intr_by_cam
    )
    return cameras, mm
    ...
    *,
    ...
    if method == "kalman":
        kalman = MultiObjectKalman(
            process_sigma=process_sigma, base_meas_sigma=base_meas_sigma, timeout_s=timeout_s
        )
    ...
    try:
        while True:
            ...
            for fr in frames:
                ...
                cv2.putText(
                    img, f"{cid}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2
                )
                cam_vis[cid] = img
                raw_models.append(
                    CameraFrameModel(
            ...
            if method == "avg":
                for oid, obs in obs_for_fuse.items():
                    ...
                    fused_now.append(
                        ObjectStateModel(
                            id=str(oid), pos_world=(float(xy[0]), float(xy[1]), 0.0), confidence=1.0
                        )
                    )
            ...
            bx0, by0 = _world_to_px(
                0.0, 0.0, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, scale=scale
            )
            bx1, by1 = _world_to_px(
                1.10, 0.40, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, scale=scale
            )
            cv2.rectangle(
                top,
...
def main() -> None:
    ...
    ap.add_argument(
        "--record-mosaic", default=None, help="Optional output mp4 containing cam0+cam1+topdown"
    )
    args = ap.parse_args()
