...
def _row(backend_name, n, k, method, samples, throughput_n, rmse, max_abs) -> BenchRow:
    return BenchRow(
        backend=backend_name, n_objects=n, obs_per_object=k, method=method,
        ...
        output_rmse_vs_cpu=rmse, max_abs_diff=max_abs,
    )
        ...
        for k in args.obs_per_object:
            ...
            for method in args.methods:
                ...
                if gpu is not None:
                    ...
                    rows.append(_row(gpu.name, n, k, method, gpu_samples, tput,
                                     round(rmse, 9), round(max_abs, 9)))
    return rows
...
def write_report(rows: list[BenchRow], path: str) -> None:
    ...
    if gpu_backends and "cpu_numpy" in backends:
        ...
        cpu = {(r.n_objects, r.obs_per_object, r.method): r.p50_ms
               for r in rows if r.backend == "cpu_numpy"}
        gpu = {(r.n_objects, r.obs_per_object, r.method): r.p50_ms
               for r in rows if r.backend == gpu_name}
        crossover_n = None
        for key in sorted(cpu.keys() & gpu.keys()):
            if gpu[key] <= cpu[key] and crossover_n is None:
                crossover_n = key[0]
        if crossover_n is not None:
            lines.append(f"GPU (`{gpu_name}`) first matches or beats CPU at "
                         f"n_objects = {crossover_n}.")
        else:
            max_n = max(r.n_objects for r in rows)
            lines.append(f"GPU (`{gpu_name}`) did **not** beat CPU at any tested size "
        ...
        lines.append("CPU-only run: no GPU available on this host, so no break-even point "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.out and not args.report:
        for r in rows:
            print(f"{r.backend}  n={r.n_objects}  obs={r.obs_per_object}  "
